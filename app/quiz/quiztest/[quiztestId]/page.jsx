...
function QuizTestPage() {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      try {
        ...
        const res = await fetch(`/api/quiz/${quiztestId}`);
        ...
        // Parse the JSON response and normalize options safely
        let parsed;
        try {
          parsed = JSON.parse(row.jsonQuizResp);
        } catch (parseErr) {
          throw new Error("Failed to parse quiz questions. The quiz data may be corrupted.");
        }

        if (!Array.isArray(parsed)) {
          // Sometimes the AI wraps questions in an object like { questions: [...] }
        ...
        const parsedQuestions = parsed.map((q) => ({
          ...q,
  ...
  const handleSubmit = () => {
    const resultData = questions.map((q, index) => {
      ...
      // Try to match the correct answer from options
      let correctOptionText = null;
      if (q.answer) {
        const answerStr = String(q.answer).trim();
        // First try exact match
        correctOptionText = q.options.find(
          (opt) => opt.trim() === answerStr
        );
        // Then try startsWith match
        if (!correctOptionText) {
          correctOptionText = q.options.find((opt) =>
            opt.trim().startsWith(answerStr)
          );
        }
        // Then try includes match (for cases like "A) Answer text" where answer is "A")
        if (!correctOptionText) {
          correctOptionText = q.options.find(
            (opt) =>
              opt.trim().toLowerCase().includes(answerStr.toLowerCase())
          );
        }
        // Fallback to the raw answer string
        if (!correctOptionText) {
          correctOptionText = answerStr;
        }
      }
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-lg">Loading your quiz...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center px-4">
        <div className="bg-zinc-900/50 backdrop-blur-sm rounded-2xl border border-rose-500/30 p-8 max-w-md text-center">
          <AlertCircle className="w-12 h-12 text-rose-400 mx-auto mb-4" />
          <h2 className="text-xl font-bold text-white mb-2">Failed to Load Quiz</h2>
          <p className="text-gray-400 mb-6">{error}</p>
          <button
            onClick={() => router.push("/quiz")}
            className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-semibold transition-all"
          >
            Create New Quiz
          </button>
        </div>
